...
const PostDetails = () => {
  ...
  return (
    <div className="w-full h-custom px-10 sm:px-14">
      ...
      <div className={`${deletestate ? "flex flex-col gap-4 items-center" : "hidden"} rounded-md absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-[40%] w-[60%] bg-gray-100`}>
      <div className="flex justify-end w-full px-4 items-center h-12">
        <button onClick={()=>setdeletestate(false)} className="text-3xl"><RxCrossCircled/></button>
      </div>
        <h1 className="text-2xl ">Do you really wants to delete this Hissab</h1>
        <div className="flex gap-4">
        <button onClick={()=>deletefnc()} className="bg-red-400 h-10 w-40 rounded-md text-white text-lg font-semibold hover:bg-red-500">Yes</button>
        <button onClick={()=>setdeletestate(false)} className="bg-sky-400 h-10 w-40 rounded-md text-white text-lg font-semibold hover:bg-sky-500">No</button>
        </div>
        </div>
        {check ? (
          <div className="relative w-full h-full ">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-48 w-40 bg-green-400">
              <form
              ...
              >
                ...
                />
                {errors.hissabPass && (
                  <p className="text-red-500">{errors.hissabPass.message}</p>
                )}
                <button
                  type="submit"
          ...
          <div>
            {post ? (
              <div className="flex flex-col gap-4 select-none">
                ...
                />
                <div className="flex gap-4">
                {post.isEditable ? <button onClick={()=>Editfnc()} className="bg-green-400 h-10 w-40 rounded-md text-white text-lg font-semibold hover:bg-green-500">Edit Hisaab</button> : null}
                <button onClick={()=>setdeletestate(true)} className="bg-red-400 h-10 w-40 rounded-md text-white text-lg font-semibold hover:bg-red-500">Delete </button> 
                {post.isShareable ? <button onClick={()=>handleShare()} className="bg-sky-400 h-10 w-10 flex justify-center items-center text-lg font-semibold rounded-md text-white hover:bg-sky-500"><IoShareSocial/></button> : null}
                </div>
              </div>
            ) : (
